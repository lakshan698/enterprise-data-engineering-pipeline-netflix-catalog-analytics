...
* **Dynamic Notebooks & Widgets:** Designed reusable, dynamic PySpark notebooks utilizing Databricks `dbutils.widgets`. This allowed the passing of parameters to dynamically process auxiliary datasets without hardcoding values.
* **Data Cleaning:** Implemented robust PySpark transformations to handle missing values, trim whitespace, remove unwanted line breaks, and drop corrupted rows.
...
* **SCD Type 2 Implementation (The Highlight):** Engineered a complex  tracking for the dim_titles table. Used Delta Lake's MERGE INTO along with an MD5 hash to detect changes, mark old rows as inactive, and add the updated version.
